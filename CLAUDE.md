...
`supabase/migrations/0001_baseline.sql` is a **baseline snapshot** of the live database (tables, RLS, functions, triggers, storage buckets/policies, cron jobs), dumped via the read-only Supabase MCP connection — not a from-scratch build script, and not tested against an empty database. **It is frozen as of 2026-09-09 and must never be edited again**, even to fix a bug the baseline itself captured or to fold in a later column/function/trigger — a snapshot that stays "current" is a snapshot of nothing, and running it followed by 0002 onward against an empty database would double-apply anything folded back into it. Every migration after it (0002, 0003, ...) is incremental on top of that frozen snapshot, including ones that touch something 0001 already defined. Before it existed, only two tables/functions the app depends on had migrations checked in at all (`profile_locations`, `unavailability_requests`, `notifications`, plus a `profiles.role` rework); everything else had been created directly via the Supabase dashboard/SQL editor with no record in this repo. The live schema can still have drifted since the baseline was captured — don't assume it's exactly current.
...
`supabase/functions/invite-staff` is the one Edge Function: it verifies the caller is a Manager, uses the service-role key to invite a user by email (`auth.admin.inviteUserByEmail`), upserts their `profiles` row and `profile_locations`. This is the only place a service-role key is used inside the deployed app — everything else goes through the anon key + RLS. The one other place a service-role key is used at all is `tests/rls/`, the automated RLS test suite, and only ever locally via a gitignored `.env.test` — see its README section before touching that suite.
